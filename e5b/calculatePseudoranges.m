function [pseudoranges] = calculatePseudoranges(trackResults, ...
    msOfTheSignal, ...
    channelList, settings,firstSubFrame)
...
% maxfirstSubFrame = max(firstSubFrame(channelList));
% addsample(channelList) = -(firstSubFrame(channelList)-maxfirstSubFrame)*0.02*4096000;
% for i=1:length(channelList)
%     a = addsample(channelList(i));
%     trackResults(channelList(i)).absoluteSample=trackResults(channelList(i)).absoluteSample+a;
% end
