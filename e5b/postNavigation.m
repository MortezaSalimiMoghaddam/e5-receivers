...
[PageStart, activeChnList, invertSymbol] = findPreambles1(trackResults);
...
for currMeasNr = 1:fix((settings.msToProcess - max(PageStart)) / ...
...
%% Find pseudoranges ======================================================
    navSolutions.channel.rawP(:, currMeasNr) = calculatePseudoranges(...
            trackResults, ...
            PageStart + settings.navSolPeriod * (currMeasNr-1), ...
...
% % [firstSubFrame, activeChnList,decodebits] = findPreambles1(trackResults);
% %% Decode ephemerides =====================================================
% % eph = zeros(length(activeChnList), )
...
%         ephemeris(decodebits(channelNr,:),firstSubFrame(channelNr));
% 
%     if eph(trackResults(channelNr).PRN).dn == 0
%         nonValidActive = [nonValidActive channelNr];
%     end
% 
...
%     %     end
% end
% activeChnList = setdiff(activeChnList, nonValidActive);
% %% Check if the number of satellites is still above 3 =====================
% % if (isempty(activeChnList) || (size(activeChnList, 2) < 4))
% %     % Show error message and exit
% %     disp('Too few satellites with ephemeris data for postion calculations. Exiting!');
...
% 
% %% Initialization of current measurement ==================================
% for currMeasNr = 1:fix((settings.msToProcess - max(firstSubFrame)) / ...
%         settings.navSolPeriod)
% 
...
%     navSolutions.channel.rawP(:, currMeasNr) = calculatePseudoranges(...
%         trackResults, ...
%         firstSubFrame + settings.navSolPeriod * (currMeasNr-1), ...
%         activeChnList, settings,firstSubFrame);
% 
%     %% Find satellites positions and clocks corrections =======================
%     [satPositions, satClkCorr] = satpos1(transmitTime, ...
%         [trackResults(activeChnList).PRN], ...
%         eph, settings);
%     %       navSolutions.channel.x(:, currMeasNr) =   satPositions(1,:);
%     %       navSolutions.channel.xy(:, currMeasNr) =   satPositions(2,:);
%     %       navSolutions.channel.z(:, currMeasNr) =   satPositions(3,:);
%     %% Find receiver position =================================================
% 
%     % 3D receiver position can be found only if signals from more than 3
%     % satellites are available
...
%     %         navSolutions.U(currMeasNr)           = NaN;
%     %
%     %         navSolutions.channel.azif(activeChnList, currMeasNr) = ...
%     %                                              NaN(1, length(activeChnList));
%     %         navSolutions.channel.el(activeChnList, currMeasNr) = ...
%     %                                              NaN(1, length(activeChnList));
% 
